import { PresetData } from '../../types';

export const meltingGrid2: PresetData = {
  name: "Melting Grid 2",
  desc: "Thermal curl turbulence with surface tension and math seed",
  params: {
    usePhysics: true,
    physicsInfluence: 1,
    clampMode: false,
    stabilityThreshold: 1,
    fadeOutRate: 0.8,
    rgbPostProcessing: {
      exposure: 1,
      contrast: 1,
      gamma: 1,
      saturation: 1,
      brightness: 0,
      tint: { r: 1, g: 1, b: 1 }
    },
    renderStyle: "pixelated",
    renderOrder: [
      "fractal",
      "gravity",
      "physics",
      "grayScott",
      "multiDim",
      "gol",
      "lga",
      "walker",
      "media",
      "soca",
      "advection"
    ],
    Dm: 0.1,
    Dc: 1,
    Dw: 5,
    kOn: 0.05,
    kRec: 0.08,
    kSat: 0.05,
    kOff: 0.8,
    feedRate: 0,
    totalDensity: 6,
    boundaryType: "periodic",
    flowX: 0,
    flowY: 0,
    flowScale: 1,
    useWalker: false,
    jitterChance: 0,
    jitterStrength: 0.5,
    noise: 0.02,
    walkerMask: false,
    walkerMaskColor: "#ffffff",
    walkerMaskTol: 0.1,
    walkerMaskInvert: false,
    useGravity: false,
    gravityStrength: 0.5,
    gravityAngle: 0,
    gravityFriction: 0.9,
    gravityMassThreshold: 2,
    useGoL: false,
    golBirth: [3],
    golSurvive: [2, 3],
    golInfluence: 0.5,
    useSoCA: false,
    socaDamping: 0.96,
    socaSpring: 0.01,
    socaDtScale: 1,
    socaSmoothness: 0.5,
    socaSmoothnessEnabled: false,
    socaReactionMix: 1,
    useLGA: false,
    lgaProbability: 0.5,
    lgaAdvection: 1,
    lgaViscosity: 0.1,
    lgaBarrier: 8,
    lgaNoise: 0.05,
    lgaFlowX: 0,
    lgaFlowY: 0,
    lgaWallColor: "#ff0000",
    lgaWallTol: 0.2,
    lgaInfluence: 1,
    lgaVerticalFactor: 1,
    useFractal: false,
    fractalDepth: 2,
    fractalBlockSize: 4,
    fractalBirth: [3],
    fractalSurvive: [2, 3],
    fractalInfluence: 0.4,
    fractalThreshold: 0.4,
    useGrayScott: false,
    gsDa: 1,
    gsDb: 0.5,
    gsFeed: 0.055,
    gsKill: 0.062,
    gsTimeScale: 1,
    gsClamp: true,
    gsInfluence: 1,
    useMultiDim: false,
    coupling: 0.2,
    multiDimZoom: 1,
    multiDimCrossDiff: 0,
    multiDimInfluence: 1,
    brushStrength: 1,
    dt: 0.1,
    colorMap: "custom"
  },
  effects: [
    {
      id: "fx_turbulence_1787073218673_m1848",
      type: "turbulence",
      name: "Curl Turbulence",
      enabled: true,
      isMinimized: false,
      params: {
        turbScale: 0.015,
        turbSpeed: 0.4,
        turbStrength: 0.6,
        turbDirX: 3,
        turbDirY: 3,
        turbFeedback: 1,
        turbOctaves: 2,
        turbInfluence: 0.05,
        influence: 0.05
      }
    },
    {
      id: "fx_surfaceTension_1787122370177_94zdx",
      type: "surfaceTension",
      name: "Surface Tension",
      enabled: true,
      isMinimized: false,
      params: {
        surfaceMobility: 0.25,
        interfacialTension: 0.59,
        phaseSeparation: 0.9,
        coalescenceRate: 2,
        tensionInfluence: 1
      }
    }
  ],
  seedConfig: {
    type: "perlin",
    intensity: 1,
    seedTarget: { u: 1, v: 0, w: 0 },
    randomThreshold: 0.005,
    perlinScale: 20,
    perlinThreshold: 0.45,
    perlinOctaves: 4,
    perlinSeed: 8476,
    perlinGradient: false,
    gridSpacingX: 20,
    gridSpacingY: 20,
    gridDotSize: 2,
    gridOffset: false,
    shapeType: "circle",
    shapeMode: "single",
    shapeCount: 1,
    shapeSize: 20,
    shapeHollow: false,
    shapePosX: 0.5,
    shapePosY: 0.5,
    mathExpression: "Math.sin(x*0.1)*Math.cos(y*0.1) > 0",
    textString: "McRD",
    textSize: 40,
    textPosX: 0.5,
    textPosY: 0.5
  },
  stabilizeConfig: {
    enabled: false,
    targetDensity: 6,
    strength: 1,
    adjustKOff: true,
    adjustKRec: true,
    adjustKOn: false,
    adjustFeed: false
  },
  automation: [],
  continuousSeeds: [
    {
      id: "cseed_initial_perlin",
      name: "Initial Perlin Seed",
      type: "perlin",
      enabled: true,
      isMinimized: true,
      opacity: 1,
      blendMode: "add",
      x: 0,
      y: 0,
      scaleX: 1,
      scaleY: 1,
      rotation: 0,
      blendIf: {
        enabled: false,
        points: [
          { pos: 0, val: 0, id: "1" },
          { pos: 1, val: 1, id: "2" }
        ],
        smoothness: 0.1
      },
      isStartingSeed: true,
      seedConfig: {
        type: "math",
        intensity: 1,
        randomThreshold: 0.05,
        perlinScale: 20,
        perlinThreshold: 0.45,
        perlinOctaves: 4,
        perlinSeed: 9791,
        perlinGradient: false,
        gridSpacingX: 10,
        gridSpacingY: 10,
        gridDotSize: 2,
        gridOffset: false,
        shapeSize: 20,
        shapeHollow: false,
        seedTarget: { u: 1, v: 0, w: 0 },
        mathExpression: "Math.sin(x*0.1)*Math.cos(y*0.1) > 0"
      }
    }
  ],
  customColorConfig: {
    mode: "scalar",
    scalarGradient: [
      { pos: 0, color: "#000000" },
      { pos: 0.35, color: "#8b5cf6" },
      { pos: 0.7, color: "#06b6d4" },
      { pos: 1, color: "#ffffff" }
    ],
    rgbMultipliers: { r: 1, g: 1, b: 1 },
    rgbBias: { r: 0, g: 0, b: 0 }
  }
};
